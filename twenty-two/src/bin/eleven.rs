...
impl MonkeyTreehouse {
    pub fn run(&mut self, idx: usize) {
        while let Some(item) = self.monkeys[idx].items.pop_front() {
            ...
            m.inspections += 1;
            let new = (m.operation)(item) / 3;
            if (m.test)(new) {
                let res = m.blocks[1];
                self.monkeys[res].items.push_back(new);
            } else {
                let res = m.blocks[0];
                self.monkeys[res].items.push_back(new);
            }
...
impl From<&str> for Monkey {
    fn from(s: &str) -> Self {
        ...
        let operation = Box::new(move |num| -> u32 {
        ...
        let t = lines.next().unwrap().to_string();
        let test = Box::new(move |num: u32| -> bool {
            num % t
                .split("divisible by ")
                .nth(1)
                .unwrap()
...
fn main() {
    ...
    (0..rounds).for_each(|_| {
        (0..monkey_treehouse.monkeys.len()).for_each(|m| {
            ...
            // dbg!(&monkey_treehouse);
        });
    });
    
    let mut monkeys = monkey_treehouse.monkeys;
    monkeys.sort();
    let res: u32 = monkeys.into_iter().rev().take(2).map(|m| m.inspections).product();
    dbg!(res);
}
